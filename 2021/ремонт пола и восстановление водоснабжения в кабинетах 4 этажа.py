...
context = {'ikz':"212621500152762150100100480000000244",
           'work' : "ремонту пола и восстановлению водоснабжения в кабинетах 4-го этажа",
           'work_name': "Ремонт пола и восстановление водоснабжения в кабинетах 4-го этажа",
           ...
           'summ' : f"{round_sum} ({num2text(sum, male_units).rsplit(' ', 1)[0]}) {num2text(sum, male_units).rsplit(' ', 1)[-1]} {sum_kop} {num2text(sum_kop, female_units).rsplit(' ', 1)[-1]}",
           'nds' : f"в том числе НДС {nds}% - {'{0:,}'.format(int(sum_nds)).replace(',', ' ')} ({num2text(sum_nds, male_units).rsplit(' ', 1)[0]}) {num2text(sum_nds, male_units).rsplit(' ', 1)[-1]} {sum_nds_kop} {num2text(sum_nds_kop, female_units).rsplit(' ', 1)[-1]}",
